...
const setup = async(conn: mariadb.Connection) => {
    ...
    const dataList = parse(fs.readFileSync(`./src/test/share/repository/setup-horse_master.csv`), {
        columns: true
    }) as HorseMaster[];
...
const selectAllOk1 = async() => {
    ...
    try {
        ...
        const csvData = parse(fs.readFileSync(`./src/test/share/repository/setup-horse_master.csv`), {
            columns: true
        }) as HorseMaster[];
